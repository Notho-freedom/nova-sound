 ...
 */
async function makeAPIRequest(baseUrl, params, secret, method = 'POST') {
    // Add signature
 ...
 */
async function getSessionKey(service, token) {
    const apiUrl = service === 'lastfm' ? LASTFM_API_URL : LIBREFM_API_URL;
    const params = {
...
async function scrobbleTrack(track) {
    ...
    if (scrobbled) {
        ...
        const localTrack = library.find(t => t.artist.toLowerCase() === track.artist.toLowerCase() &&
...
async function processScrobbleQueue() {
    ...
    for (let i = queue.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            if (item.duration)
                params.duration = item.duration.toString();
 ...
 */
export function shouldScrobble(playedSeconds, trackDuration) {
...
export function initScrobbler() {
    ipcMain.handle('scrobbler:scrobble', async (_event, track) => {
        return scrobbleTrack(track);
    });
    ipcMain.handle('scrobbler:nowPlaying', async (_event, track) => {
        return updateNowPlaying(track);
    });
    ipcMain.handle('scrobbler:status', async () => {
        return getStatus();
    });
    ipcMain.handle('scrobbler:authenticate', async (_event, service) => {
        return authenticate(service);
    });
    ipcMain.handle('scrobbler:disconnect', async (_event, service) => {
