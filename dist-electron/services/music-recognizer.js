 ...
 */
function findSimilarArtist(artist, existingTracks) {
 ...
 */
function findSimilarAlbum(album, artist, existingTracks) {
 ...
 */
function detectPatterns(unknownTracks) {
 ...
 */
export async function recognizeTrack(track, existingTracks) {
 ...
 */
export async function recognizeAllUnknownTracks() {
    const allTracks = await storage.getLibrary();
    const unknownTracks = allTracks.filter(t => t.artist === 'Artiste inconnu' || t.album === 'Album inconnu');
    const results = [];
    for (const track of unknownTracks) {
 ...
 */
export async function detectAndGroupPatterns() {
    const allTracks = await storage.getLibrary();
    const unknownTracks = allTracks.filter(t => t.artist === 'Artiste inconnu' || t.album === 'Album inconnu');
...
export function initMusicRecognizer() {
    ...
    ipcMain.handle('recognize:apply', async (_event, results) => {
    ...
    ipcMain.handle('recognize:apply-group', async (_event, group) => {
    ...
    ipcMain.handle('recognize:track', async (_event, trackId) => {
...
//# sourceMappingURL=music-recognizer.js.map