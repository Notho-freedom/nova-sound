...
function saveSecondaryWindowState(key, bounds) {
    try {
        let state = {};
        try {
...
function fadeInWindow(win, duration = 180) {
    try {
        win.setOpacity(0);
        const steps = 12;
        const delta = 1 / steps;
 ...
 */
function createSecondaryWindow(title, key, overrides = {}) {
    const baseDefaults = {
        width: 640,
        height: 800,
        minWidth: 480,
...
ipcMain.handle('stripe:openWindow', async (_event, url) => {
    try {
        ...
        if (urlObj.hostname === 'localhost' && urlObj.port === '3001') {
            ...
            if (code && state) {
                ...
                if (mainWindow && !mainWindow.isDestroyed()) {
                    ...
                    app.pendingOAuthCallback = { code, state };
...
function handleStripeRedirect(url) {
    if (!url)
        return;
    try {
        const urlObj = new URL(url);
...
function registerOAuthCallbackServer() {
    ...
    oauthCallbackServer.listen(PORT, '127.0.0.1', () => {
        const address = oauthCallbackServer?.address();
...
app.whenReady().then(async () => {
    ...
    if (app.pendingOAuthCallback) {
        const { code, state } = app.pendingOAuthCallback;
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('oauth:callback', { code, state });
            delete app.pendingOAuthCallback;
        }
    }
    ...
    if (app.pendingFiles) {
        const pendingFiles = app.pendingFiles;
        console.log('═══════════════════════════════════════════════════════════');
