...
async function getLyrics(
  ...
  try {
    ...
    const response = await fetchJSON<LyricsSearchResult>(url);
 ...
 */
async function getLyricsById(id: number): Promise<LyricsResult | null> {
  try {
...
export function findCurrentLyricsLine(
  ...
  currentTime: number,
  offset: number = 0
): { currentIndex: number; currentLine: LyricsLine | null; nextLine: LyricsLine | null } {
...
export function initLyricsProvider() {
  ...
  ipcMain.handle('lyrics:getWithAlbum', async (
    _event,
    artist: string,
    title: string,
    album: string,
