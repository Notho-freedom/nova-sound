...
import { storage, type Playlist } from './storage.js';

/**
 * Parse M3U/M3U8 playlist file
 */
async function parseM3U(filePath: string): Promise<string[]> {
 ...
 */
async function parsePLS(filePath: string): Promise<string[]> {
  const content = await fs.readFile(filePath, 'utf-8');
  const lines = content.split(/\r?\n/);
  const trackPaths: string[] = [];
  const baseDir = path.dirname(filePath);
 ...
 */
function generateM3U(tracks: Array<{ filePath: string; title: string; artist: string; duration: number }>): string {
  const lines = ['#EXTM3U'];
 ...
 */
function generatePLS(tracks: Array<{ filePath: string; title: string; artist: string; duration: number }>): string {
 ...
 */
async function importPlaylist(filePath: string): Promise<Playlist | null> {
 ...
 */
async function exportPlaylist(playlistId: string, format: 'm3u' | 'pls'): Promise<string | null> {
  try {
...
async function createSmartPlaylist(
  ...
  switch (criteria.type) {
    case 'artist':
      filteredTracks = library.filter(t => 
        t.artist.toLowerCase().includes((criteria.value || '').toLowerCase())
      );
      break;
    case 'album':
      filteredTracks = library.filter(t => 
        t.album.toLowerCase().includes((criteria.value || '').toLowerCase())
      );
      break;
    case 'genre':
      filteredTracks = library.filter(t => 
        t.genre?.toLowerCase().includes((criteria.value || '').toLowerCase())
      );
      break;
    case 'year':
      ...
      break;
    case 'recent':
      filteredTracks = [...library].sort((a, b) => 
...
export function initPlaylistManager() {
  ipcMain.handle('playlists:import', async (_event, filePath: string) => {
    return importPlaylist(filePath);
  });

  ipcMain.handle('playlists:export', async (_event, playlistId: string, format: 'm3u' | 'pls') => {
    return exportPlaylist(playlistId, format);
  });

  ipcMain.handle('playlists:createSmart', async (_event, name: string, criteria: any) => {
    return createSmartPlaylist(name, criteria);
  });

  ipcMain.handle('playlists:addTracks', async (_event, playlistId: string, trackIds: string[]) => {
    const playlist = await storage.getPlaylist(playlistId);
  ...
  ipcMain.handle('playlists:removeTracks', async (_event, playlistId: string, trackIds: string[]) => {
    const playlist = await storage.getPlaylist(playlistId);
    if (!playlist) return null;
  ...
  ipcMain.handle('playlists:duplicate', async (_event, playlistId: string) => {
    const playlist = await storage.getPlaylist(playlistId);
    if (!playlist) return null;
