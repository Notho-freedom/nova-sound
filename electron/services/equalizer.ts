...
export function initEqualizer() {
  ...
  ipcMain.handle('equalizer:setEnabled', async (_event, enabled: boolean) => {
    await storage.updateSettings({ equalizerEnabled: enabled });
    return enabled;
  });

  // Apply a preset
  ipcMain.handle('equalizer:applyPreset', async (_event, presetName: string) => {
    const presets = await storage.getEqualizerPresets();
  ...
  ipcMain.handle('equalizer:setBands', async (_event, bands: number[]) => {
    await storage.updateSettings({ 
  ...
  ipcMain.handle('equalizer:setBand', async (_event, index: number, value: number) => {
    const settings = await storage.getSettings();
 ...
 */
export function interpolateEQCurve(bands: number[], numPoints: number = 100): { frequency: number; gain: number }[] {
